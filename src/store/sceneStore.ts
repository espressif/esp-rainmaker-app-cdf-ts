...
class SceneStore {
  ...
  #interceptProxy(scene: Scene) {
    ...
    const updateNodeScene = ({
      ...
      if (operation === SceneOperation.EDIT) {
        ...
        scenes.splice(
          scenes.findIndex((scene: any) => scene.id === id),
          1
        );
      }
      const nodeConfig = {
  ...
  #transformNodeListToScenes = (
    ...
    return nodeList.reduce(
      (acc, node) => {
        ...
        if (!scene) return acc;
        const sceneList =
          scene.params.find((param: any) => param.type === ESPRM_PARAM_SCENES)
            ?.value || [];
        sceneList.forEach((sceneData: any) => {
          if (acc[sceneData.id]) {
            ...
            acc[sceneData.id].devicesCount += Object.keys(
              sceneData.action
            ).length;
          } else {
            // Create new scene
  ...
  @action createScene = async (sceneData: Partial<Scene>): Promise<Scene> => {
    try {
      const devicesCount = Object.values(sceneData.actions || {}).reduce(
        (acc: number, deviceCofig: any) => {
          acc += Object.keys(deviceCofig).length;
          return acc;
        },
