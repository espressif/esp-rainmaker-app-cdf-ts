...
import { SUCCESS, NodeUpdateType, ESPRM_SERVICE_SCHEDULES, ESPRM_PARAM_SCHEDULES, ScheduleOperation } from "../utils/constants";
...
class ScheduleStore {
    ...
    beforeSetScheduleListHook: (schedules: Schedule[]) => void = (schedules: Schedule[]) => { };

    /** Hook called after setting schedule list - for customization */
    afterSetScheduleListHook: (schedules: Schedule[]) => void = (schedules: Schedule[]) => { };
    ...
    #interceptProxy(schedule: Schedule) {
        ...
        const updateNodeSchedule = (param: updateNodeScheduleParams) => {
            const { nodeId, action, name, info, id, triggers, flags, validity, operation } = param;
            ...
            let scheduleIndex = schedules.findIndex((schedule: any) => schedule.id === id);
     ...
     */
    isScheduleInSync = (existingSchedule: Schedule, scheduleData: any, keys: string[]): { isInSync: boolean, meta?: Record<string, any> } => {
        let isInSync = true;
        ...
        keys.forEach(key => {
            ...
            } else if (existingValue !== newValue && newValue !== null && newValue !== undefined) {
                isInSync = false;
                outOfSyncMeta[key] = newValue;
            }
        });
     ...
     */
    #transformNodeListToSchedules = (nodeList: ESPRMNode[]): { [key: string]: Schedule } => {
        return nodeList.reduce((acc, node) => {
            ...
            const scheduleList = schedule.params.find((param: any) => param.type === ESPRM_PARAM_SCHEDULES)?.value || [];
            scheduleList.forEach((scheduleData: any) => {
                if (acc[scheduleData.id]) {
                    ...
                    acc[scheduleData.id].devicesCount += Object.keys(scheduleData.action).length;

                    const { isInSync, meta } = this.isScheduleInSync(acc[scheduleData.id], scheduleData, ['name', 'enabled', 'triggers', 'validity', 'flags', 'info']);
                    if (!isInSync) {
     ...
     * });
     */
    @action createSchedule = async (scheduleData: Partial<Schedule>): Promise<Schedule> => {
        try {
            const devicesCount = Object.values(scheduleData.action || {}).reduce((acc: number, deviceCofig: any) => {
                acc += Object.keys(deviceCofig).length;
                return acc;
            }, 0);
    ...
    */
    public setScheduleList = (schedules: Schedule[]) => {
        this._schedulesByID = schedules.reduce((acc, schedule) => {
    ...
    */
    ...
    @action syncSchedulesFromNodes = async (nodeIds: string[]): Promise<void> => {
        try {
            this.clear();
            const nodeList = this.rootStore?.nodeStore.nodeList.filter((node) => nodeIds.includes(node.id)) || [];
