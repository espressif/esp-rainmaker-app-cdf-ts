...
class Scene implements SceneInterface {
    ...
    constructor(scene: Scene, rootStore: CDF) {
        ...
        this.edit = (args: { name: string, actions: any, info?: string }) => this.#edit(args);
    ...
    * 
    */
    #generatePayload(actions: any, nodeId: string, type: SceneOperation, sceneName?: string, sceneId?: string, info?: string): any {
        const sceneData: Record<string, any> = {
            [SceneOperation.ADD]: { id: sceneId, operation: SceneOperation.ADD, name: sceneName, action: actions, info: info },
            [SceneOperation.ACTIVATE]: { id: sceneId, operation: SceneOperation.ACTIVATE },
            [SceneOperation.REMOVE]: { id: sceneId, operation: SceneOperation.REMOVE },
            [SceneOperation.EDIT]: { name: sceneName, id: sceneId, operation: SceneOperation.EDIT, action: actions, info: info }
        };
    ...
    #operations = (type: SceneOperation, sceneName: string = this.name, actions: any = this.actions, nodes: string[] = this.nodes, info: string = this.info || '') => {
        ...
        const payload = nodes.map((nodeId: string) => {
            ...
            if (type === SceneOperation.EDIT) {
                ...
                return this.#generatePayload(actions[nodeId], nodeId, operationType, sceneName, this.id, info);
            }

            // Handle other operation types directly
            return this.#generatePayload(actions[nodeId], nodeId, type, sceneName, this.id, info);
        });
