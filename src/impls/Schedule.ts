import { Schedule as ScheduleInterface, ESPAPIResponse, CDF, ScheduleEditParams, ScheduleTrigger, ScheduleAction, SchedulePayload, ScheduleOperationParams, ScheduleGeneratePayloadParams, MultiNodeResponsePayload } from "../types/index";
...
class Schedule implements ScheduleInterface {
    ...
    #generatePayload(
        ...
        const { action, nodeId, triggers, scheduleId, scheduleName, info, flags, validity } = param || {};
     ...
     */
    #determineEditOperation(nodeId: string, newaction: any, scheduleName: string, triggers: ScheduleTrigger[], info: string | null | undefined, flags: number | null | undefined, validity: { start: number, end: number } | null | undefined): ScheduleOperation {
        if (!nodeId) {
        ...
        if (existInOldaction && existInNewaction && !isEqual(this.action[nodeId], newaction[nodeId])) {
     ...
     */
    #operations = (type: ScheduleOperation, param?: ScheduleOperationParams): Promise<ESPAPIResponse | undefined | MultiNodeResponsePayload[]> => {
        const { scheduleName = this.name, triggers = this.triggers, action = this.action, nodes = this.nodes, info = this.info, flags = this.flags, validity = this.validity } = param || {};
        ...
        const payload = nodes.reduce((acc: any, nodeId: string) => {
            if (type === ScheduleOperation.EDIT) {
                ...
                const operationType = this.#determineEditOperation(nodeId, action, scheduleName, triggers, info, flags, validity);

                if (operationType === ScheduleOperation.NO_CHANGE) {
                    return acc;
                }
                this.callbackUpdateOperation[nodeId] = operationType;
                acc.push(this.#generatePayload(
                    operationType,
                    {
    ...
    */
    ...
    validateSchedule = ({ name, triggers, action }: Schedule) => {
        ...
        if (!triggers.every(trigger => trigger.m !== undefined || trigger.rsec !== undefined)) {
            throw new Error("Every trigger must have either minutes (m) or relative seconds (rsec) defined");
        }
        if (!action) {
